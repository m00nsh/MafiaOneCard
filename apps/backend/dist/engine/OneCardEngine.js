...
exports.OneCardEngine = void 0;
const shared_1 = require("@mafia/shared");
class OneCardEngine {
    constructor(state, deck, turnManager) {
        this.state = state;
        this.deck = deck;
    ...
    processCardPlay(sessionId, cardId, selectedSuit) {
        const player = this.state.players.get(sessionId);
        if (!player) {
            return {
                success: false,
                error: { code: shared_1.ErrorCode.INTERNAL_SERVER_ERROR, message: "Player not found" }
            };
        }
        // 1. 카드 찾기
        ...
        if (cardIndex === -1) {
            return {
                success: false,
                error: { code: shared_1.ErrorCode.CARD_NOT_IN_HAND, message: "Hand does not contain this card" }
            };
        }
        ...
        if (this.state.attackStack > 0) {
            ...
            if (attackValue === 0 || attackValue < currentAttackValue) {
                return {
                    success: false,
                    error: {
                        code: shared_1.ErrorCode.MUST_RESPOND_TO_ATTACK,
        ...
        if (topCard && topCard.id !== "") {
            ...
            if (this.state.selectedSuit && this.state.selectedSuit !== "") {
                targetSuit = this.state.selectedSuit;
            }
            ...
            if (!canPlay) {
                return {
                    success: false,
                    error: {
                        code: shared_1.ErrorCode.INVALID_CARD_SUIT,
        ...
        this.state.topCard = new shared_1.CardSchema(card.id, card.suit, card.rank);
    ...
    applyCardEffects(card, selectedSuit) {
        ...
        if (attackVal > 0) {
            // 공격 카드를 낸 경우 스택 추가
            if (card.rank === 'A')
                this.state.attackStack += shared_1.GAME_CONSTANTS.ATTACK_A;
            else if (card.rank === '2')
                this.state.attackStack += shared_1.GAME_CONSTANTS.ATTACK_2;
            else if (card.suit === 'JOKER' && card.rank === 'BLACK')
                this.state.attackStack += shared_1.GAME_CONSTANTS.ATTACK_JOKER_BLACK;
            else if (card.suit === 'JOKER' && card.rank === 'COLOR')
                this.state.attackStack += shared_1.GAME_CONSTANTS.ATTACK_JOKER_COLOR;
        }
        else {
            // 일반 카드를 낸 경우 스택 초기화
...
exports.OneCardEngine = OneCardEngine;
