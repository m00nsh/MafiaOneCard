...
class SkillManager {
    constructor(state, deck, turnManager, engine // 승리 조건 체크 등 활용 가능
    ) {
        this.state = state;
        this.deck = deck;
     ...
     */
    canBeShamanTarget(targetPlayer) {
        const skillId = targetPlayer.characterId;
        if (!skillId)
            return false;
    ...
    // 스킬 사용 메인 진입점
    useSkill(sessionId, skillId, targetId, selectedCardId, targetIds) {
        const player = this.state.players.get(sessionId);
        if (!player)
            return { success: false, error: { code: shared_1.ErrorCode.INTERNAL_SERVER_ERROR, message: "Player not found" } };
    ...
    validateSkillUse(sessionId, player, skillId) {
        // 내 턴인지 확인
        if (this.state.currentTurn !== sessionId) {
            return { success: false, error: { code: shared_1.ErrorCode.NOT_YOUR_TURN, message: "Not your turn" } };
        }
        ...
        if (skillId === 'summoner' || skillId === 'berserker') {
            if (player.skillUsesLeft <= 0) {
                return { success: false, error: { code: shared_1.ErrorCode.INTERNAL_SERVER_ERROR, message: "No skill uses left" } };
            }
        }
        else {
            // 나머지는 게이지 방식 (skillProgress >= skillMaxCooldown)
            if (player.skillProgress < player.skillMaxCooldown) {
                return { success: false, error: { code: shared_1.ErrorCode.INTERNAL_SERVER_ERROR, message: "Skill on cooldown" } };
            }
        }
        // 캐릭터 일치 여부 확인 (보안)
        if (player.characterId !== skillId && player.characterId !== 'summoner') {
            ...
            return { success: false, error: { code: shared_1.ErrorCode.INTERNAL_SERVER_ERROR, message: "Character mismatch" } };
        }
        return { success: true };
    }
    executeSkill(sessionId, player, skillId, targetId, selectedCardId, targetIds) {
        // 결과 모음 (승리/파산 체크용)
        let result = { success: true };
        const affectedPlayers = new Set(); // 변경된 플레이어 목록
        affectedPlayers.add(sessionId);
        switch (skillId) {
            case 'merchant': // 잡상인: 내 카드 1장 -> 타겟
                if (!targetId || !selectedCardId)
                    return { success: false, error: { message: "Target and Card required" } };
                const targetM = this.state.players.get(targetId);
                const cardIndex = Array.from(player.hand).findIndex(c => c.id === selectedCardId);
                if (!targetM || cardIndex === -1)
                    return { success: false, error: { message: "Invalid target or card" } };
                // 이동
            ...
            case 'tank': // 탱커: 누적 데미지 -2, 남은거 드로우
                if (this.state.attackStack <= 0)
                    return { success: false, error: { message: "No attack to block" } };
                ...
                if (this.state.attackStack > 0) {
                    ...
                    for (let i = 0; i < this.state.attackStack; i++) {
                        ...
                        if (card) {
                            const cardSchema = new shared_1.CardSchema(card.id, card.suit, card.rank);
            ...
            case 'thief': // 도둑: 이전/다음 플레이어의 패에서 카드 한 장씩 랜덤으로 뽑아 주고 받음
                ...
                if (prevId === sessionId || nextId === sessionId)
                    break; // 혼자일때
            ...
            case 'shaman': // 주술사: 타겟 지정 (강제 스킬, 거부권 없음)
                if (!targetId)
                    return { success: false, error: { message: "Target required" } };
                const targetS = this.state.players.get(targetId);
                if (!targetS)
                    return { success: false, error: { message: "Invalid target" } };
            ...
            case 'summoner': // 소환사: 타겟 스킬 복사 사용
                ...
                if (!targetSummon)
                    return { success: false, error: { message: "Invalid target" } };
                const copiedSkill = targetSummon.characterId;
                if (!copiedSkill)
                    return { success: false, error: { message: "Target has no skill" } };
                // 1. Validation (복사 가능 여부 체크)
                const skillInfo = shared_1.CHARACTER_SKILLS[copiedSkill];
            ...
            case 'assassin': // 암살자: 타겟에게 3장 부여
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    if (c) {
                        const cardSchema = new shared_1.CardSchema(c.id, c.suit, c.rank);
            ...
            case 'berserker': // 광전사: 나 1장, 타겟들 3장
                ...
                if (opponentCount >= 2) {
                    if (uniqueTargets.size < 2)
                        return { success: false, error: { message: "Must select 2 targets" } };
                }
                else if (opponentCount === 1) {
                    if (uniqueTargets.size < 1)
                        return { success: false, error: { message: "Must select 1 target" } };
                }
                // 2. Self Draw 1
                if (this.deck.count === 0)
                    this.deck.replenish();
                    ...
                    uniqueTargets.forEach(tid => {
                        ...
                        if (t && tid !== sessionId) { // 본인 제외 안전장치
                            for (let i = 0; i < 3; i++) {
                                ...
                                if (c2) {
                                    const cardSchema = new shared_1.CardSchema(c2.id, c2.suit, c2.rank);
    ...
    checkImmediateWinOrBurst(playerIds) {
        const eliminated = [];
        let gameEnded = false;
        for (const pid of playerIds) {
            ...
            // 파산 조건 (>20장)
            if (handCount > shared_1.GAME_CONSTANTS.MAX_HAND_SIZE) {
                eliminated.push(pid);
            }
        }
     ...
     */
    checkSummonTarget(targetId) {
        const targetPlayer = this.state.players.get(targetId);
        if (!targetPlayer)
            return { success: false, error: "Invalid target" };
        const skillId = targetPlayer.characterId;
        if (!skillId)
            return { success: false, error: "Target has no skill" };
